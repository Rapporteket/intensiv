...
#								'3' = as.character(RegData$ShNavn[match(reshID, RegData$ReshId)]))
...
if (enhetsUtvalg %in% c(0,2,4,7)) {		#Ikke sammenlikning
			...
			smltxt <- switch(as.character(enhetsUtvalg),
				'1' = 'landet forøvrig',
				'3' = paste('andre ', SykehustypeTxt[RegData$ShType[indEgen1]], sep=''),	#RegData inneh. kun egen shgruppe
...
if (retn == 'H') {
	...
	if (medSml == 1 & enhetsUtvalg==3) {
	  points(as.numeric(rev(Andeler$Rest)), pos, col=fargeRest,  cex=2, pch=18) #c("p","b","o"), 
	  legend('top', c(paste(shtxt, ' (N=', Nhoved,')', sep=''), paste(shTypetext,' forøvrig (N=', Nrest,')', sep='')), 
	         border=c(fargeSh,NA), col=c(fargeSh,fargeRest), bty='n', pch=c(15,18), pt.cex=2, 
	         lwd=lwdRest,	lty=NA, ncol=1, cex=cexleg)
	}  
...
>>>>>>> c79a859b34ee39c3fde00288b039d0f8b4ac9aed

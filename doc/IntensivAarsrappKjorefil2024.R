...
tabNokkeltall <- rbind(
  ...
  'Mekanisk \nventilasjonsstøtte (%)' = tapply(RegData$respiratortid>0, RegData$EnhNivaa,
                                               FUN=function(x) round(sum(x, na.rm=T)/length(x)*100,1)),

  'Respiratordøgn, \nsamlet (median)' = tapply(RegData$respiratortid[indRespt], RegData$EnhNivaa[indRespt],
                                               FUN=median, na.rm=T),
  'Respiratordøgn, \ninvasiv (median)' = tapply(RegData$InvasivVentilation[indRespInv], RegData$EnhNivaa[indRespInv],
                                                FUN=median, na.rm=T),
  'Respiratordøgn, \nnon-invasiv (median)' = tapply(RegData$NonInvasivVentilation[indRespNIV], RegData$EnhNivaa[indRespNIV],
                                                    FUN=median, na.rm=T),
  ...
  'Reinnleggelser, \n<72t (%)' = tapply(RegDataReinn$Reinn==1, RegDataReinn$EnhNivaa,
                                        FUN=function(x) round(sum(x, na.rm=T)/length(x)*100,1)),
  'Utskrevet \n kl 17-08 (%)' = tapply(RegData$Ut1708, RegData$EnhNivaa,
                                       FUN=function(x) round(sum(x, na.rm=T)/length(x)*100,1)),
  ...
  'Intermitt. hemodia. v/nyreerstat.' = tapply(RegData$Intermitterende[indNyre], RegData$EnhNivaa[indNyre],
                                                  FUN=function(x) round(sum(x, na.rm=T)/length(x)*100,1)),
  'Intermitt. hemodia. v/nyreerstat.' = tapply(RegData$IntermitterendeDays[indNyre], RegData$EnhNivaa[indNyre],
                                               FUN=median, na.rm=T),

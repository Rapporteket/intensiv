...
Pas$Totaltid_start <- as.Date(Pas$Totaltid_start)
...
ggplot2::ggplot(data = Pas, mapping = ggplot2::aes(x=Aldersgruppe, fill=Gruppe)) + ggplot2::geom_bar(position = 'identity', alpha=0.5)
...
#                                col=c(1,4),lwd=c(1,1),lty=c(1,1), lab.x="dager fra operasjon",lab.y="andel dod", 
#                               legend=TRUE, local.x=0, local.y=0.4,legend.0="gr1",legend.1="gr2")
...
# fit <- survminer::surv_fit(Surv(time, status) ~ sex, data = survival::colon)
# fit <- survminer::surv_fit(Surv(time, status) ~ sex,
...
Pas$Gruppe01fa <- factor(Pas$Gruppe01, levels = 0:1, labels = c('gr1', 'gr2'))
...
ggplot2::ggplot(data = Inngr, mapping = ggplot2::aes(x=Pri5gr, fill=Gruppe)) + ggplot2::geom_bar(position = 'identity', alpha=0.5)

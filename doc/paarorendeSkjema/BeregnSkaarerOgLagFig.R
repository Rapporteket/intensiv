...
for (valgtVar in Skaar) {
      outfile <- paste0(valgtVar, '.png')
      NIRFigGjsnPrePostPaaror(RegData=RegData, valgtVar=valgtVar, outfile=outfile)
}
valgtVar <- 'SymptomUroSkaar'
#RegData$PrePost
NIRFigGjsnPrePostPaaror  <- function(RegData, valgtVar, valgtMaal='Gjsn', prePost=2, grVar='ShNavn',outfile='')	
{

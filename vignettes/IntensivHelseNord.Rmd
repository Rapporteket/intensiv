...
# for (aar in 2018:2020) {
 ...
 write.table(cbind(TabManglReg, Pst), file = paste0('DatakvalSMR_Nord.csv'), sep = ';')
...
sykehusnavn <- c('Helse Nord RHF', sort(unique(RegData$HelseenhetKortnavn)))
...
for (sykehus in sykehusnavn) {
  ...
  Tab <- kableExtra::kbl(Tab,  digits= 1, #format='html', #align=c('l', rep('r', ncol(tabNokkeltall))), #row.names=F,
...
```{r Datakvalitet, echo=FALSE, results = 'asis'} 
RegData$

...
*Skal også ha med eksempler på hvor stor andel av totalantallet som må fjernes*
...
KvalIndGrVar <- c('erMann', 'ShNavn', 'Aar')
Ngrense <- 5

AndelMistet <- function (RegData, KvalIndGrVar, Ngrense){}                  

#Tre grupperingsvariable: År, enhet, kjønn:
...
#ident_ut <-test2[ind_faa,KvalIndGrVar]
...
Gruppering                                | N<5 | N<3
------------------------------------------|------|------|
Enhet, år, kjønn:                         | `r round(AndelBort,1)` | 8|
Enhet, år, kjønn, 10-årige aldersgrupper  | 3 |5|
Enhet, måned, kjønn                       | 2|4|
Enhet, år, kjønn, aldersgr., innmåte, død | 1|2|

Med alder menes 10-årigealdersgrupper p




```{r Eks1, echo=FALSE}
print(test2[ind_faa,], row.names=FALSE)
```

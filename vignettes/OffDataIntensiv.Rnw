...
Eksemplet under viser hendelsesentydige data for  en kvalitetsindikatorer fra intensivregisteret. Kvalitetsindikatoren er gitt i kolonna \textit{Variabel}.
...
FrafallVSgruppering <- rbind(
      ...
      'Enhet, måned, kjønn' = c(N5_Mnd2var, N3_Mnd2var),
      'Enhet, kjønn, aldersgruppe, innmåte, død' = c(N5_6var, N3_6var),
      'Enhet, kvartal, kjønn, aldersgruppe' = c(N5_Kvartal2var, N3_Kvartal2var)
)
...
FrafallVSgruppering2000 <- rbind(
      ...
      'Enhet, måned, kjønn' = c(N5_Mnd2var2000, N3_Mnd2var2000),
      'Enhet, kjønn, aldersgruppe, innmåte, død' = c(N5_6var2000, N3_6var2000),
      'Enhet, kvartal, kjønn, aldersgruppe' = c(N5_Kvartal2var2000, N3_Kvartal2var2000)
)
...
\item Kjønn (mann/kvinne)
\item År og kvartal (tidsangivelse for innleggelsestidspunkt)
\end{itemize}

\textit{Reidar: Disse har jeg valgt, men du må si hvilke utvalg det er mest aktuelt å prioritere? Enhet må vi ha. Sykehustype lager ingen ekstra gruppering ift. antall. Jeg mener også vi må ha med kvartal for å kunne vise tidsaktuelle data.}
\\

Alle registreringer som faller i ei gruppe med færre enn 5 observasjoner i gruppa, er sensurert. F.eks. hvis utvalgskombinasjonen: Sykehus A, aldersgruppe 2, kvinne, 2.kvartal 2016 inneholder færre enn 5 observasjoner, sensureres alle observasjonene i denne gruppa. Som vi ser fra tabellen med grad av sensur for intensivregisteret (tabell \ref{tab:Frafall}), betyr det at \Sexpr{N5_Kvartal2var} \% av registreringene blir sensurert med de valgte grupperingsvariablene. 
...
DataTilbakeResp <- NIRAndelerGrVar(RegData=RegData, valgtVar = 'respiratortidInv', grVar='ShNavn',preprosess=0, outfile = 'FigResp.pdf')

sykehus <- 'Ringerike' #
indB <- which(names(DataTilbakeResp$Ngr$Hoved) == sykehus)
RespRes <- sprintf('%.1f',DataTilbakeResp$AggVerdier$Hoved[indB])

DataTilbakeRespOff <- NIRAndelerGrVar(RegData=NIRdata01respiratortidInv, aar=2016, valgtVar = 'respiratortidInv', grVar='ShNavn', preprosess=0, offData = 1, outfile = 'FigRespSens.pdf')

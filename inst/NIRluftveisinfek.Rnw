...
knitr::opts_chunk$set(warnings=FALSE,echo=FALSE, dev = "svglite")
...
LuftData$RHFc <- as.character(LuftData$RHF)
  ...
  # levels = format(seq(min(LuftData$InnDato), max(LuftData$InnDato), by='day'), '%d.%m.%y'))
   ...
   # LuftData$Innleggelsestidspunkt <- as.POSIXlt(LuftData$Innleggelsestidspunkt, tz= 'UTC', format='%Y-%m-%d %H:%M:%S' )
...
Dokumentet viser resultater fra ferdigstilte opphold med påvist luftveisinfeksjon. (Opphold i kladd er ikke med.) At et opphold er ferdigstilt vil si at opplysningene er kontrollert og ferdig registrert.

Variablene som inngår i luftveisrapporteringen, ble innført i uke 42, 2025. Enkelte sykehus har imidlertid registrert data tilbake i tid. Rapporteringen vil derfor først være komplett fra og med uke 42, 2025.


\bf{Merk at:}
\begin{itemize}
\item Rapporten er basert på registreringer 40 uker bakover i tid til og med \Sexpr{Sys.Date()-1}.
\item Bare uker hvor det er registrerte tilfeller, vises i tabellene. (Uker med 0 registreringer vises ikke.)
\end{itemize}
...
\begin{frame}[fragile] {Antall opphold med luftveisinfeksjoner per HF}
% Når overføring av ikke-ferdigstilte opphold til Rapporteket er på plass, skal denne skal endres til å vise inneliggende-

I tabellen filtreres det på pasienter som er innlagt på grunn av luftveisinfeksjon.
...
print(xtable::xtable(RegHF, digits=0,
                     align = c('l', 'l', 'l', 'r'),
                     caption = 'Antall luftveispasienter med luftveisinfeksjon som hovedårsak til innleggelse'),
...
#skaler <- ifelse(dim(TabUkeRHF)[1]>25, 1- 0.006*dim(TabUkeRHF)[1] ,1)
  ...
  \begin{frame}[fragile]{Opphold med luftveisinfeksjon på hver enhet}
  ...
  skaler <- ifelse(dim(RegSh)[1]>25, 1- 0.007*dim(RegSh)[1] ,1)
  print(
...
@
  ...
  %\scalebox{0.5}
  \centering
  \includegraphics[scale=0.38]{AlderKjonn.pdf}
  \caption{Fordeling av kjønn i ulike aldersgrupper for opphold med registrert luftveisinfeksjon
  siste 40 uker. Søylene maskeres hvis det er færre enn tre kvinner eller menn i ei aldersgruppe.}
  \end{figure}

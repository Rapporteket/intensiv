...
library(RMySQL)
library(xtable)
#kontakt <- dbConnect(MySQL(), user="nir", password="nir", dbname="nir", host='172.19.15.124')  #'localhost'
kontakt <- dbConnect(MySQL(), user="ro_IRRDS", password="fropFav2", dbname="IntensivRegisterReportDataStaging", host='172.19.13.111')
...
spørring.nir <- 'SELECT FreshOrganizationUnitId AS avdid, 			
						...
						DischargedHospitalStatus AS utstatus,
...
utstatus <- as.data.frame(cbind('Ved utskrivelse fra sykehus'=c('ikke registrert','i livet','død intensiv','død post','reinnlagt intensiv'), Avdeling=0, Lokalsykehus=0, Regionalsykehus=0, Sentralsykehus=0))
for(i in 2:5) {utstatus[,i] <- as.numeric(as.character(utstatus[,i]))}
score <- c(0,0,1,2,3)
for(i in 1:dim(utstatus)[1]) {
  if(i==1) {
    utstatus$Avdeling[i] <- round(length(which(is.na(nir$utstatus)))/length(nir$utstatus)*100,1)
    utstatus$Lokalsykehus[i] <- round(length(which(is.na(lokal$utstatus)))/length(lokal$utstatus)*100,1)
    utstatus$Regionalsykehus[i] <- round(length(which(is.na(regional$utstatus)))/length(regional$utstatus)*100,1)
    utstatus$Sentralsykehus[i] <- round(length(which(is.na(sentral$utstatus)))/length(sentral$utstatus)*100,1)

  } else {
    utstatus$Avdeling[i] <- round(length(which(nir$utstatus==score[i]))/length(nir$utstatus)*100,1)
    utstatus$Lokalsykehus[i] <- round(length(which(lokal$utstatus==score[i]))/length(lokal$utstatus)*100,1)
	utstatus$Regionalsykehus[i] <- round(length(which(regional$utstatus==score[i]))/length(regional$utstatus)*100,1)
	utstatus$Sentralsykehus[i] <- round(length(which(sentral$utstatus==score[i]))/length(sentral$utstatus)*100,1)
  }
}
print(xtable(utstatus, 
             align=rep('r', ncol(utstatus)+1),
			 digits=1, 
			 caption='Pasientenes status ved utskrivelse fra sykehus angitt som prosentandeler.', 
			 label="tab:UtStatus"), 
	 include.rownames=FALSE)
...
variabler <- c('TypeOfAdmission','TransferredStatus','PatientTransferredFromHospital','PatientTransferredToHospital','DischargedHospitalStatus','ChronicDiseases','Glasgow','Age','SystolicBloodPressure',

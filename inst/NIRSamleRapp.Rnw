...
#knitr::options(replace.assign=TRUE, width=90)
knitr::options(replace.assign=TRUE, width=90, warnings=FALSE,echo=FALSE, fig.align='center', fig.show='hold', out.width='.8\\textwidth', cache=FALSE)
#fig.path='figurer/', 
@
...
ShTypeTxt <- c('lokal/sentral','lokal/sentral', 'regional')
...
#NIRdata$ShNavn <- as.factor(NIRdata$ShNavn)	#Hvis dette legges på, får vi ikke valgt ut bare sykehustypens sykehus
...
ind <- list(Eget = which(NIRdata$ReshId == reshID),
			EgenShType = which(NIRdata$ShType == ShTypeEgen),
			Alle = 1:N)
...
#ShTypeEgen <- as.character(NIRdata[which(NIRdata$ReshId == reshID),'ShTypeTxt'])[1]
...
tabAvdNEget <- rbind(
	...
	'Registrerte pasienter' = table(aggregate(erMann~Aar+PasientGUID,data=NIRdataEget,length)$Aar),	#length(table(NIRdataEget$PasientId)),
...
#dum <- Fordeling(RegData=NIRdata, valgtVar='alder', grType='alle', datoFra=datoFra, datoTil=datoTil, 
...
dum <- NIRFigGjsnGrVar(RegData=NIRdata, valgtVar='alder', valgtMaal='', preprosess=0, hentData=0, 
                       grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigAlderGjsn.pdf')
dum <- NIRFigGjsnGrVar(RegData=NIRdata, valgtVar='alder', valgtMaal='Med', preprosess=0, hentData=0, 
                       grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigAlderMed.pdf')
@
...
tabKjonnShType <- round(100*rbind(
	...
	'Lokal-/Sentralenheter' = table(NIRdata[which(NIRdata$ShType==1), 'erMann'])/length(which(NIRdata$ShType==1)),	
	'Regionalenheter' = table(NIRdata[which(NIRdata$ShType==3), 'erMann'])/length(which(NIRdata$ShType==3)),	
...
#		grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigAlderK.pdf')
#dum <- Fordeling(RegData=NIRdata, valgtVar='alder', preprosess=0, hentData=0, erMann=1,
#		grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigAlderM.pdf')
...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigAlderM.pdf} 
 \caption{Aldersfordelig blant hhv. kvinner og menn ved \Sexpr{shtxt} mot 
	\Sexpr{paste(ShTypeEgenTxt,'sykehus', sep='')} forøvrig.}	
  \label{fig:AlderKM}
\end{figure}
...
tabLiggLokSentr <- table(NIRdata[which(NIRdata$ShType==1), 'LiggetidGr'])
tabLiggReg <- table(NIRdata[which(NIRdata$ShType==3), 'LiggetidGr'])
tabLiggAlle <- table(NIRdata$LiggetidGr)

tabLiggetidShType <- round(100*rbind(
#	'Egen enhet' = tabLiggEget/sum(tabLiggEget),	
	'Lokal-/sentralenheter' = tabLiggLokSentr/sum(tabLiggLokSentr),		
...
#dum <- Fordeling(RegData=NIRdata, valgtVar='liggetid', grType='alle', datoFra=datoFra, datoTil=datoTil, 
...
dum <- NIRFigGjsnGrVar(RegData=NIRdata, valgtVar='liggetid', valgtMaal='Mean', preprosess=0, hentData=0,  
			grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigLiggetidGjsn.pdf')
dum <- NIRFigGjsnGrVar(RegData=NIRdata, valgtVar='liggetid', valgtMaal='Med', preprosess=0, hentData=0,  
			grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigLiggetidMed.pdf')
@
...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigLiggetidMed.pdf} 
 \caption{Gjennomsnittlig og median liggetid ved \Sexpr{paste(ShTypeEgenTxt,'sykehus', sep='')}.}	
...
dum <- NIRFigAndelerGrVar(RegData=NIRdata, valgtVar='reinn', preprosess=0, hentData=0, grType=ShTypeEgen, datoFra=datoFra, 
...
\begin{figure}[ht] 
  ...
  \caption{Andel reinnleggelser for alle \Sexpr{paste(ShTypeEgenTxt,'sykehus', sep='')}}
  \label{fig:ReinnEgen}
\end{figure}
...
tabTypeOpphLokSentr <- table(NIRdata[which(NIRdata$ShType==1), 'InnMaate'])
...
tabTypeOpphShType <- round(100*rbind(
	...
	'Lokal-/Sentralenheter' = tabTypeOpphLokSentr/sum(tabTypeOpphLokSentr),		
...
dum <- NIRFigAndelerGrVar(RegData=NIRdata, valgtVar='innMaate', preprosess=0, hentData=0, grType=ShTypeEgen, datoFra=datoFra, 
	datoTil=datoTil, outfile='FigTypeOpph.pdf') 
@

\begin{figure}[ht] 
  \centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigTypeOpph.pdf} 
  \caption{Fordeling av type opphold for \Sexpr{paste(ShTypeEgenTxt,'sykehusenheter.', sep='')}}
  \label{fig:TypeOpph}
\end{figure}
...
NIRdataUtv <- NIRdata[indNEMS, c('NEMS', 'ShType','liggetid','ReshId')]
...
tabNEMSShType <- round(rbind(
	...
	'Lokal-/Sentralenheter' = c(NEMS_N[1], NEMSgjsn[1], NEMSmed[1], NEMSgjsnD[1], NEMSmedD[1]),
...
dum <- NIRFigGjsnGrVar(RegData=NIRdata, valgtVar='NEMS', valgtMaal='Mean', preprosess=0, hentData=0,  
			grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigNEMS.pdf')
@

\begin{figure}[ht] 
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigNEMS.pdf} 
    \caption{Gjennomsnittlig NEMS per døgn for \Sexpr{paste(ShTypeEgenTxt,'sykehus.', sep='')}}
	\label{fig:NEMS}
\end{figure}
...
NIRdataUtv <- NIRdata[indNas, c('Nas', 'NasD', 'ShType','liggetid','ReshId')]
indEgenUtv <- which(NIRdataUtv$ReshId == reshID)
...
tabNasShType <- round(rbind(
	...
	'Lokal-/Sentralenheter' = c(Nas_N[1], Nasgjsn[1], Nasmed[1], NasgjsnD[1], NasmedD[1]),
...
NIRdataUtv <- NIRdata[indResp, c('respiratortid', 'ShType','ReshId')]
indEgenUtv <- which(NIRdataUtv$ReshId == reshID)
...
tabRespShType <- rbind(
	...
	'Lokal-/Sentralenheter' = c(RespLokSentr[4], RespLokSentr[3], AndelResp[2], paste(RespLokSentr[1],' - ',RespLokSentr[6], sep='')),
...
#dum <- Fordeling(RegData=NIRdata, valgtVar='respiratortid', grType=ShTypeEgen, 
...
dum <- NIRFigGjsnGrVar(RegData=NIRdata, valgtVar='respiratortid', valgtMaal='Mean', preprosess=0, hentData=0,  
		grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigResptidGjsn.pdf') 
dum <- NIRFigGjsnGrVar(RegData=NIRdata, valgtVar='respiratortid', valgtMaal='Med', preprosess=0, hentData=0,  
		grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigResptidMed.pdf') 
@


\begin{figure}[ht] 
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigResptidFord.pdf} 
  \caption{Fordeling av respiratortid ved \Sexpr{paste(ShTypeEgenTxt,'sykehus', sep='')}}
...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigResptidMed.pdf} 
 \caption{Gjennomsnittlig og median respiratortid ved \Sexpr{paste(ShTypeEgenTxt,'sykehus', sep='')}.}	
  \label{fig:ResptidMM}
\end{figure}
...
#dum <- Fordeling(RegData=NIRdata, valgtVar='SAPSII', grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, 
...
dum <- NIRFigGjsnGrVar(RegData=NIRdata, valgtVar='SAPSII', valgtMaal='Mean', preprosess=0, hentData=0,  
		grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigSAPSgjsn.pdf') 
@
...
SAPSII for \Sexpr{ShTypeEgenTxt}sykehus. 
...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigSAPSgjsn.pdf} 
 \caption{Fordeling av SAPS II og gjennomsnittlig SAPS II ved \Sexpr{paste(ShTypeEgenTxt,'sykehus.', sep='')}.}	
  \label{fig:SAPSII}
\end{figure}
...
dum <- NIRFigGjsnGrVar(RegData=NIRdata, valgtVar='SMR', valgtMaal='Mean', preprosess=0, hentData=0,  
		grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigSMR.pdf') 
@
\begin{figure}[ht] 
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigSMR.pdf} 
  \caption{Standardisert mortalitetsratio, \Sexpr{paste(ShTypeEgenTxt,'sykehus.', sep='')}}
  \label{fig:SMR}
\end{figure}
...
dum <- NIRFigGjsnGrVar(RegData=NIRdata, valgtVar='liggetid', valgtMaal='Mean', preprosess=0, hentData=0, dodInt=1, 
		grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigLiggetidDode.pdf') 	#LiggetidDodGjsn.pdf
@
...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigLiggetidDode.pdf} 
  \caption{Gjennomsnittlig liggetid på intensiv for ikke-overlevere, \Sexpr{paste0(ShTypeEgenTxt,'sykehus.')}}

...
\section{Oversikt for \Sexpr{shtxt} }
 ...
 \Sexpr{min(NIRdataEget$InnDato)} til \Sexpr{max(NIRdataEget$InnDato)}.
...
\begin{figure}[ht] 
  ...
  \caption{Andel reinnleggelser for alle \Sexpr{paste0(ShTypeEgenTxt,'sykehus')}}
  \label{fig:ReinnEgen}
\end{figure}
...
dum <- NIRFigAndelerGrVar(RegData=NIRdata, valgtVar='dodeIntensiv', preprosess=0, hentData=0, grType=ShTypeEgen, 
...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigLiggetidDode.pdf} 
  \caption{Andel som dør på intensiv og median liggetid på intensiv for disse, \Sexpr{paste0(ShTypeEgenTxt,'sykehus.')}}

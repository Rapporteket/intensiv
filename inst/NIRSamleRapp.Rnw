...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigAlderMed.pdf} 
 \caption{Median alder ved \Sexpr{paste0(ShTypeEgenTxt,'sykehus.')}}	
...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigAlderM.pdf} 
 \caption{Aldersfordelig blant hhv. kvinner og menn ved \Sexpr{shtxt} mot 
	\Sexpr{paste0(ShTypeEgenTxt,'sykehus')} forøvrig.}	
  \label{fig:AlderKM}
\end{figure}
...
liggetidDIV <- cbind('Egen enhet'= summary(NIRdataEget$liggetid)[1:6],
			'Resten av landet'= summary(NIRdataResten$liggetid)[1:6])
rownames(liggetidDIV) <- c('Minimum:', '25% av pasientene er under:', 'Median:','Gjennsomsnitt:',
		'25% av pasientene er over:', 'Maksimum:')


xtable(liggetidDIV, digits=1, align=c('l', rep('r', ncol(liggetidDIV))), 
		caption='Liggetidskarakteristikker',
...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigLiggetidMed.pdf} 
 \caption{Median liggetid ved \Sexpr{paste0(ShTypeEgenTxt,'sykehus')}.}	
...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigNEMS.pdf} 
    \caption{Median NEMS per døgn for \Sexpr{paste0(ShTypeEgenTxt,'sykehus.')}}
	\label{fig:NEMS}
\end{figure}
...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigResptidMed.pdf} 
 \caption{Median respiratortid ved \Sexpr{paste0(ShTypeEgenTxt,'sykehus')}.}	
  \label{fig:ResptidMM}
\end{figure}
...
dum <- NIRFigGjsnGrVar(RegData=NIRdata, valgtVar='SAPSII', valgtMaal='Med', preprosess=0, hentData=0,  
		grType=ShTypeEgen, datoFra=datoFra, datoTil=datoTil, outfile='FigSAPSmed.pdf') 
@
...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigSAPSmed.pdf} 
 \caption{Fordeling av SAPS II og median SAPS II ved \Sexpr{paste0(ShTypeEgenTxt,'sykehus.')}.}	
  \label{fig:SAPSII}
\end{figure}
...
\par
SMR er observert 30-dagers mortalitet delt på SAPS II-estimert mortalitet for opphold 
der pasienten faktisk er SAPS II-skåret. 
...
For reinnlagte er SMR kun knyttet til 30-dagers mortalitet ved første opphold (f.eks. 
...
Man må tolke SMR-verdiene varsomt, 
...
\centering \includegraphics[width= \Sexpr{figKrymp1}\textwidth]{FigLiggetidDode.pdf} 
  \caption{Median liggetid på intensiv for ikke-overlevere, \Sexpr{paste0(ShTypeEgenTxt,'sykehus.')}}

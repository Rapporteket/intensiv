...
datoFra <- '2016-01-01'	
datoNorsk <- format.Date(datoFra, format='%d.%m%.%Y')
if (!exists('NIRdata')){
...
Denne samlerapporten er basert på alle opphold som er lagret og ferdigstilt i NIR fra og med \Sexpr{datoFraNorsk} %oppstart 
inntil dagen før 
...
tabAvdNEget <- rbind(
	...
	'Registrerte pasienter' = table(aggregate(erMann~Aar+PasientID,data=NIRdataEget,length)$Aar),	#length(table(NIRdataEget$PasientId)),

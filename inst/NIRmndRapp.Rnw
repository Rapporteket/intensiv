...
rappMnd <- paste0(mndTxt[dato$mon+1], ' ', dato$year+1900)
...
xtable(liggetidKj, digits=1, align=c('l', rep('r', ncol(liggetidKj))), 
		caption='Liggjedøger og andel kvinner, siste 6 månader.',
...
\caption{Hovudårsak til innlegging på intensiv (prosent per kategori) sist månad, eiga avdeling og landet elles.}
\end{figure}
\end{frame}
...
finnGjsnMedNEMS <- function(RegData){
      ...
      Antal <- dim(RegData)[1]
      Ut <- c(GM_NEMS, GM_NEMSd[2:1], Antal)
      }
...
colnames(NEMStab) <- c('Gj.sn/opph', 'Median/opph','Median/døger', 'Gj.sn/døger', 'Antal')

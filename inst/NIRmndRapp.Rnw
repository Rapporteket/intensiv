...
#Enhetsnivå
nivaaEgen <-  RegData$NivaaNum[match(reshID, RegData$ReshId)]
ShNivaaEgenTxt <- c('overvåknings', 'postoperative', 'Gen<50pst','Gen>50pst', 'barne')[nivaaEgen]
...
#	outfile <- paste0(valgtVar, 'FordMnd', '.pdf')
	NIRFigAndeler(RegData=RegData, valgtVar='PrimaryReasonAdmitted',
	              datoFra=datoFra, reshID=reshID,
	              enhetsUtvalg=3, preprosess=0,
	              outfile = 'PrimaryReasonAdmittedFordMnd.pdf')
# }
...
\caption{Hovudårsak til innlegging på intensiv (prosent per kategori) siste seks månader,
eiga avdeling og andre \Sexpr{ShNivaaEgenTxt} einingar}
\end{figure}
\end{frame}
...
finnDivRespMaal <- function(RegData){
      ...
      Resp <- sprintf('%1.1f', summary(RegData$respiratortid[indResp]))

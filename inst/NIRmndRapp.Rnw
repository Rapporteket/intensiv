...
Denne rapporten kan hentast ved at ein loggar seg inn på Rapporteket-Intensiv og klikkar på "Månedsrapport". 
Det gjer at alle kan hente seg representative månadsrapportar når dei måtte ynskje. En kan óg abbonera på rapporten. 
Ein vél da sjølve kor ofte ein ynskjer rapporten.
I Rapporteket kan ein óg finne mange fleire figurar og tabellar og gjere endringar i tidsintervall og samanlikningsgrunnlag.
...
tabBelegg <- tabBelegg(RegData=RegDataEget, datoTil=datoTil, tidsenhet=tidsenhet) #personIDvar='PasientID', 

...
\textbf{NB: Lena skal sortere og legge på navn på måneder.}
...
Registeret mangler variabel for å beregne dette, så denne tas bort inntil videre.
(FirstTimeClosed)
...
xtable(alderDIV, digits=1, align=c('l', rep('r', ncol(alderDIV))), 
		...
		label='tab:Alderskarakteristikker, siste seks månader.')
...
xtable(liggetidKj, digits=1, align=c('l', rep('r', ncol(liggetidKj))), 
		caption='Liggjetid og andel kvinner',
...
\begin{frame}[fragile] {Dobbeltregistreringar}

Feilsjekk, mulige dobbeltregistreringer:  
...
print(kable(tabDbl, row.names = FALSE))
      #,caption='Sannsynlige dobbeltregistreringer.', label='tab:Dbl')

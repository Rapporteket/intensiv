...
datoFraMnd <- dato - as.Date(paste0(1900+dato$year,'-', ifelse(dato$mon==11, 1, dato$mon+2), '-', '01'))
...
enhetsUtvalg <- 1 #SJEKK OM STANDARD
tidsenhet <- 'Mnd'
preprosess <- 0
hentData <- 0
...
\begin{frame}[fragile] {Oversikt for nn (NIR-eining)}

Oversikt for nn (NIR-eining) som i samlerapport, men med tal på opphald, pasientar og døger for dei siste månadene (med siste månad som den rapporten primært gjeld)
Ved nn er det registrert innleggingar for perioden ….. til …….åå
jan 	feb 	mars 	april 	mai	 juni
Ferdigstilte intensivopphald  33 	35 	18 	24 	19 	23
Registrerte pasientar 
Liggjetid (tal på intensivdøger) 

\caption{Opphald, pasientar og liggjetid (intensivdøger)}
\end{frame}

\begin{frame}[fragile] {Registreringsforseinking}
Har registeret en variabel som gjør at dette kan beregnes?
(FirstTimeClosed)
...
for (valgtVar in variable) {
	...
	NIRFigAndeler(RegData=RegData, valgtVar=valgtVar, datoFra=datoFra, reshID=reshID, 
	               enhetsUtvalg=enhetsUtvalg, preprosess=0)
}
...
Hovudårsak til intensivopphaldet (mal av figur 1 i samlerapport)
Histogram med vertikale søyler i% av total i dei 11 hovudkategoriane. Eiga avd. søyler, resten av landet prikkar
11
 
Figur 2: Hovudårsak til innlegging på intensiv (% per kategori) eiga avdeling og landet elles
...
\caption{Opphald (\%) der det vart utført trakeostomi eller gjeve nyrerstattande behandling i 

...
NIRUtvalg <- NIRUtvalgEnh(RegData, reshID = reshID, datoFra = datoFra6mnd, datoTil = datoTil, 
                          enhetsUtvalg =3, grType = grType)
...
maxDatoEget <- format.Date(max(RegDataEget$InnDato), format='%d.%m%.%Y')
...
Denne rapporten er utarbeidet på oppdrag frå Norsk intensivregister (NIR) og genereres automatisk
...
\caption{Hovudårsak til innlegging på intensiv (prosent per kategori) siste seks månader, eiga avdeling og 
\Sexpr{paste0(ShTypeEgenTxt,'sjukehus')} forøvrig.}
\end{figure}
\end{frame}

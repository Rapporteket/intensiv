...
\textbf{Av grunnane over, ber NIR om at denne rapporten ikkje blir brukt offentleg i nokon samanheng eller blir offentleggjort på noko vis.}
...
Denne rapporten er utarbeidet på oppdrag frå Norsk
intensivregister (NIR) og genereres automatisk frå Rapporteket. 
Alle resultat er baserte på data frå registeret og er gyldige per rapportdato for opphald som er ferdigstilte i MRS. For \Sexpr{shtxt} er siste ferdigstilte innleggingsdato 
\Sexpr{format.Date(max(RegDataEget$InnDato), format='%d.%m%.%Y')}.
Rapporten inneheld alle ferdigstilte opphald i MRS der 
innleggingstidspunktet på intensiv er i tidsrommet kl.00.00 den fyrste i aktuell månad til kl. 23.59 den siste i månaden. 
Uttrekket blir gjort for kvar kalendermånad, med ein knapp månads forseinking. Månadsrapport for januar vert sendt ut sist i februar, for februar sist i mars osv.
NIR-einingar som er ajour med registreringa, vil dermed kunne få nær fullstendige månadsrapportar. 
\par

Denne rapporten kan óg hentast ved at ein loggar seg inn på Rapporteket-Intensiv og klikkar på "Månedsrapport". 
Det gjer at alle kan hente seg representative månadsrapportar når dei måtte ynskje. I Rapporteket kan ein óg finne mange fleire figurar og tabeller, og gjere endringar i tidsintervall og samanlikningsgrunnlag.
...
\caption{Hovudårsak til innlegging på intensiv (prosent per kategori) siste seks månader, eiga avdeling og landet elles.}
\end{figure}
\end{frame}

...
CoroData$ShNavn <- CoroData$HealthUnitShortName
CoroData <- NIRPreprosess(CoroData, skjema = 4)
...
\textcolor{red}{Ref, influensaregistrering: "Når ein opprettar eit influensaskjema har ein per def. mistanke om influensa. Vi meiner difor at skjema med verdi -1 også bør tellast med som mistenkt influensa} 
Jeg antar dette også gjelder Corona og teller med Diagnose=-1 (ikke angitt) som mistenkt tilfelle.
...
print(xtable::xtable(TabDagRHF, digits=0, #method='compact', #align=c('l', rep('r', ncol(alderDIV))),
		caption=paste0('Intensivopphold per dag og region (B - bekreftet, M - mistenkt)')),
      #footnote= 'B-bekreftet, M-mistenkt',),
...
\begin{frame}[fragile] {tilfeller registrert siste uke}

Nye tilfeller registrert siste uke i hver helseregion. (Basert på FormDate)
...
xtable::xtable(addmargins(TabRHF), digits=0, caption='Coronaopphold registrert inneværende uke')
...
xtable::xtable(addmargins(TabRHF), digits=0, caption='Antall døde per dag i hvert RHF')
...
TabKjonn <- table(CoroData$Dag, CoroData$erMann)
...
Mogleg å lage «grove» alderskategoriar først, og forfine etter kvart som ein får fleire tilfeller? 
\textcolor{red}{Ja, det kan vi. Har nå lagt inn aldergruppesteg på 25. Det endres automatisk 
til 10 når vi passerer 100 registreringer.}
...
\begin{frame}[fragile] {Corona på respirator}
Tal CoV-pasientar som ligg på respirator
tal CoV-pasientar som ligg på ECMO. (Andel av total kapasitet?)
\textcolor{red}{Mangler info om total kapasitet for avd/respirator/ECMO}

Teller nå opp hvor mange som har vært i respirator/ECMO. Mangler visning av 
antall som ligger på respirator/ECMO akkurat nå, kommer.. 
...
TabHjelp[,'Andel'] <- paste0(sprintf('%.1f', TabHjelp[,2]),'%')
...
TabRisiko <- cbind(
Kreft = tapply(CoroData$IsCancerPatient, CoroData$Dag, FUN=sum, na.rm = T),
'Nedsatt immunforsvar' = tapply(CoroData$IsImpairedImmuneSystemIncludingHivPatient, CoroData$Dag, FUN=sum, na.rm = T),
Diabetes	= tapply(CoroData$IsDiabeticPatient, CoroData$Dag, FUN=sum, na.rm = T),
...
Astma	= tapply(CoroData$IsAsthmaticPatient, CoroData$Dag, FUN=sum, na.rm = T),
...
Graviditet	= tapply(CoroData$IsPregnant, CoroData$Dag, FUN=sum, na.rm = T),
'Røyker' =	tapply(CoroData$IsActiveSmoker, CoroData$Dag, FUN=sum, na.rm = T),
'Opphold med risikofaktorer' = tapply(CoroData$IsRiskFactor, CoroData$Dag, FUN=sum, na.rm = T)
)

xtable::xtable(t(TabRisiko),
               digits=0,
